...
parser.add_argument(
    '--experiment_root', default='./checkpoints/',
...
parser.add_argument(
    '--image_root', type=common.readable_directory,default='./data/Market-1501-v15.09.15/',
...
parser.add_argument(
    '--aggregator', choices=AGGREGATORS.keys(), default='mean',
...
def flip_augment(image, fid, pid):
    ...
    return images, tf.stack([fid,fid]), tf.stack([pid,pid])
...
data_fids =  glob.glob('./extract_images/*')
data_fids.sort()
...
dataset = dataset.map(
    lambda fid: common.fid_to_image(
        fid, tf.convert_to_tensor('dummy'), image_root='.',
...
if args.crop_augment == 'center':
    ...
    dataset = dataset.map(lambda im, fid, pid:
        (tf.stack(five_crops(im, net_input_size)), tf.stack([fid]*5), tf.stack([pid]*5)))
    dataset = dataset.apply(tf.contrib.data.unbatch())
...
if len(modifiers) > 1:
    ...
    emb_storage_agg = AGGREGATORS[args.aggregator](emb_storage)
    
    

dis_chart = np.zeros((254,254))

for i in range(len(emb_storage_agg)):
    for j in range(len(emb_storage_agg)):
        dis_chart[i,j]= np.sqrt( np.sum( np.square(emb_storage_agg[i] - emb_storage_agg[j]))+1e-12 )
